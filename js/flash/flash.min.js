...
function flashFormTogglePassword(form) {
	...
	const password = form.querySelector('.fl-form-password-input');
	const icon = form.querySelector('.fl-form-password-input-icon');
...
function flashFormGallery(form) {
		...
		const input = form.querySelector('.fl-form-gallery-file-input');

		if(!input) return;

		const gallery = flashCreateElement("DIV","", {
		...
		const createGallery = () => {
			...
			for (var i = 0, containers = []; i < event.target.files.length; i++) {
				...
				containers[i] = flashCreateElement("DIV","", {
					class: "fl-form-gallery-img-container",
				}, gallery, "last-child");
		...
		form.addEventListener('reset', resetGallery);	

}

// form-profile-image
function flashFormProfileImage(form) {
	
		// get-elements-and-values
		const input = form.querySelector('.fl-form-profile-img-file-input');
		const img = form.querySelector('.fl-form-profile-img');
		...
		// functions
		const changeImg = () => img.src = URL.createObjectURL(event.target.files[0]);
		const resetImg = () => img.src = defaultSrc;


		// adding-functions
		input.addEventListener('change', changeImg);
		form.addEventListener('reset', resetImg);	
...
function flashFormImage(form) {
		...
		// get-elements-and-values
		const input = form.querySelector('.fl-form-img-file-input');
